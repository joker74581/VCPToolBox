...
function buildSummary({ url, parsed, normalized, warnings = [], title = '# 卖家精灵选品筛选结果' }) {
  ...
  if (parsed.candidates.length > 0) {
    ...
    lines.push('| # | 商品信息 | 大类BSR | 父销量 / 增长率 | 销售额 | 子销量 / 销售额 | 变体 | 价格 / Q&A | 评分数 / 新增 | 评分 / 留评率 | FBA费 / 利润率 | 上架 / 卖家数 | 卖家 / 属性 | 类目 |');
    lines.push('|---|---|---|---|---|---|---|---|---|---|---|---|---|---|');
    parsed.candidates.forEach((item, index) => {
      const v = (val) => (val === null || val === undefined || val === 0 || val === '0' || val === '0.00%') ? '-' : val;

      let titleVal = item.title || '-';
      let brandVal = item.brand ? `[${item.brand}]` : '';
      let asinInfo = `子: ${item.asin}`;
      if (item.parent_asin) {
        asinInfo += ` / 父: ${item.parent_asin}`;
      }
      let badgesInfo = '';
      if (Array.isArray(item.badges) && item.badges.length > 0) {
        badgesInfo = `<br><small>${item.badges.map(b => `[${b}]`).join(' ')}</small>`;
      }
      const productCol = `**${titleVal}** ${brandVal}<br><small>${asinInfo}</small>${badgesInfo}`;

      let bsrVal = v(item.category_bsr);
      const bsrDetails = [];
      if (item.bsr_growth_value) bsrDetails.push(`增: ${item.bsr_growth_value}`);
      if (item.bsr_growth_rate) bsrDetails.push(`率: ${item.bsr_growth_rate}`);
      if (bsrDetails.length > 0) {
        bsrVal += `<br><small>${bsrDetails.join(' / ')}</small>`;
      }

      const parentSales = `${v(item.parent_sales)} / ${v(item.parent_sales_growth_rate)}`;
      const childSales = `${v(item.child_sales)} / ${v(item.child_revenue)}`;
      const priceCol = `${v(item.price)} / ${v(item.qa_count)}`;
      const reviewsCol = `${v(item.review_count)} / ${v(item.monthly_new_reviews)}`;
      const ratingCol = `${v(item.rating)} / ${v(item.review_rate)}`;
      const fbaCol = `${v(item.fba_fee)} / ${v(item.profit_margin)}`;
      const putawayCol = `${v(item.putaway_date)} / ${v(item.seller_count)}`;

      const sellerDetails = [];
      if (item.buybox_seller) {
        let bb = `BB: ${item.buybox_seller}`;
        if (item.buybox_location) bb += ` (${item.buybox_location})`;
        sellerDetails.push(bb);
      }
      if (item.lqs) {
        sellerDetails.push(`LQS: ${item.lqs}`);
      }
      if (item.weight_info) {
        sellerDetails.push(`属性: ${item.weight_info}`);
      }
      if (item.pkg_weight_info) {
...
function buildKeywordSummary({ url, parsed, normalized, warnings = [] }) {
  ...
  if (parsed.candidates && parsed.candidates.length > 0) {
    ...
    lines.push('| # | 关键词 / 类目与周期 | 月搜 / 日搜 | 月购买 / 购买率 | 展示 / 点击 | 增长率 | 同比 / 近3月增长 | ABA点击 / 转化 | ABA排名 | 货流值 | PPC低 / 均 / 高 | 需供比 / 商品数 | 均价 / 均评 / 评分 | SPR / 标题密度 |');
    lines.push('|---|---|---|---|---|---|---|---|---|---|---|---|---|---|');
    parsed.candidates.forEach((item, index) => {
      const v = (val) => (val === null || val === undefined || val === 0 || val === '0' || val === '0.00%') ? '-' : val;
      ...
      const details = [];
      if (item.category) details.push(`类目: ${item.category}`);
      if (item.market_period) details.push(`周期: ${item.market_period}`);
      const titleCol = details.length > 0 ? `${title}<br><small>${details.join(' | ')}</small>` : title;

      const searches = `${v(item.monthly_searches)} / ${v(item.daily_searches)}`;
      const purchases = `${v(item.monthly_purchases)} / ${v(item.purchase_rate)}`;
      const impressions = `${v(item.impressions)} / ${v(item.clicks)}`;
      const growth = `${v(item.yearly_growth)}<br>${v(item.recent_3_month_growth)}`;
      const aba = `${v(item.aba_click_share)} / ${v(item.aba_conversion_share)}`;

      const ppcMin = item.ppc_bid?.min;
      const ppcBid = item.ppc_bid?.bid;
      const ppcMax = item.ppc_bid?.max;
      const ppc = `${v(ppcMin)} / ${v(ppcBid)} / ${v(ppcMax)}`;

      const supply = `${v(item.supply_demand_ratio)} / ${v(item.products)}`;
      const market = `${v(item.avg_price)} / ${v(item.avg_reviews)} / ${v(item.avg_rating)}`;
      const spr = `${v(item.spr)} / ${v(item.title_density)}`;

      lines.push(`| ${index + 1} | ${titleCol} | ${searches} | ${purchases} | ${impressions} | ${v(item.growth_rate)} | ${growth} | ${aba} | ${v(item.aba_rank)} | ${v(item.goods_value)} | ${ppc} | ${supply} | ${market} | ${spr} |`);
    });
  } else if (parsed.no_results) {
    lines.push('');
...
function buildKeywordReverseSummary({ url, parsed, normalized, warnings = [] }) {
  ...
  if (parsed.candidates && parsed.candidates.length > 0) {
    ...
    lines.push('');
    lines.push('## 候选关键词 (合并表)');
    lines.push('| # | 关键词 / 流量类型 | 流量占比 / 周曝光 / 类别 | 自然 / 广告排名 | 月搜 / 日搜 / ABA周 | 月购买 / 购买率 | 展示 / 点击 | 需供比 / 商品数 | ABA点击 / 转化 | PPC低 / 均 / 高 | SPR / 密度 / 广告竞品 |');
    lines.push('|---|---|---|---|---|---|---|---|---|---|---|');
    parsed.candidates.forEach((item, index) => {
      const v = (val) => (val === null || val === undefined || val === 0 || val === '0' || val === '0.00%') ? '-' : val;

      let keywordCol = item.translation ? `${item.keyword}<br><small>(${item.translation})</small>` : item.keyword;
      if (item.traffic_type) {
        const tags = String(item.traffic_type).split(/\s+/).filter(Boolean);
        if (tags.length > 0) {
          keywordCol += `<br><small style="color: #666;">${tags.join(', ')}</small>`;
        }
      }

      const shareRow = `${v(item.traffic_share)} / ${v(item.traffic_count)}`;
      const classDetails = [];
      if (item.traffic_class) classDetails.push(item.traffic_class);
      if (item.conversion_effect) classDetails.push(item.conversion_effect);
      const shareCol = classDetails.length > 0
        ? `${shareRow}<br><small style="color: #666;">${classDetails.join(' | ')}</small>`
        : shareRow;

      const orgRow = item.organic_rank !== null
        ? `${item.organic_rank}${item.organic_page_info ? ` (${item.organic_page_info})` : ''}`
        : '-';
      const orgRowWithTime = item.organic_rank_time ? `${orgRow} <small style="color: #999;">${item.organic_rank_time}</small>` : orgRow;

      const adRow = item.ad_rank !== null
        ? `${item.ad_rank}${item.ad_page_info ? ` (${item.ad_page_info})` : ''}`
        : '-';
      const adRowWithTime = item.ad_rank_time ? `${adRow} <small style="color: #999;">${item.ad_rank_time}</small>` : adRow;
      const rankCol = `自: ${orgRowWithTime}<br>广: ${adRowWithTime}`;

      const searchesRow = `${v(item.monthly_searches)} / ${v(item.daily_searches)}`;
      const searchCol = item.aba_week_rank !== null
        ? `${searchesRow}<br><small style="color: #666;">ABA周: ${item.aba_week_rank}</small>`
        : searchesRow;

      const purchaseCol = `${v(item.monthly_purchases)} / ${v(item.purchase_rate)}`;
      const impressionCol = `${v(item.impressions)} / ${v(item.clicks)}`;
      const supplyCol = `${v(item.supply_demand_ratio)} / ${v(item.products)}`;
      const abaCol = `${v(item.aba_click_share)} / ${v(item.aba_conversion_share)}`;

      const ppcLow = item.ppc_bid?.low !== null && item.ppc_bid?.low !== undefined ? `$${item.ppc_bid.low}` : '-';
      const ppcMid = item.ppc_bid?.mid !== null && item.ppc_bid?.mid !== undefined ? `$${item.ppc_bid.mid}` : '-';
      const ppcHigh = item.ppc_bid?.high !== null && item.ppc_bid?.high !== undefined ? `$${item.ppc_bid.high}` : '-';
      const ppcCol = `${ppcLow} / ${ppcMid} / ${ppcHigh}`;

      const sprCol = `${v(item.spr)} / ${v(item.title_density)} / ${v(item.ad_competitors)}`;

      lines.push(`| ${index + 1} | ${keywordCol} | ${shareCol} | ${rankCol} | ${searchCol} | ${purchaseCol} | ${impressionCol} | ${supplyCol} | ${abaCol} | ${ppcCol} | ${sprCol} |`);
    });
  } else if (parsed.no_results) {
    lines.push('');
...
function buildKeywordConversionRateSummary({ url, parsed, normalized, warnings = [] }) {
  ...
  if (parsed.candidates && parsed.candidates.length > 0) {
    ...
    lines.push('');
    lines.push('## 候选关键词 (合并表)');
    lines.push('| # | 关键词 | 搜索 / 点击 / 购买 | 搜索转化 / 点击转化 | PPC / CPA (低/中/高) | 均价 / ACOS (低/中/高) | 预算 / ABA集中度 | Top3点击ASIN份额 |');
    lines.push('|---|---|---|---|---|---|---|---|');
    parsed.candidates.forEach((item, index) => {
      const v = (val) => formatCompactValue(val);
      const title = item.translation ? `${item.keyword}<br><small>(${item.translation})</small>` : item.keyword;

      const searchesCol = `${v(item.period_searches)} / ${v(item.period_clicks)} / ${v(item.period_purchases)}`;
      const convCol = `${v(item.search_conversion_rate)} / ${v(item.click_conversion_rate)}`;

      const formatBid = (obj) => {
        if (!obj) return '-/-/-';
        const low = obj.low !== null && obj.low !== undefined ? `$${obj.low}` : '-';
        const mid = obj.mid !== null && obj.mid !== undefined ? `$${obj.mid}` : '-';
        const high = obj.high !== null && obj.high !== undefined ? `$${obj.high}` : '-';
        return `${low}/${mid}/${high}`;
      };
      ...
      const formatPrice = (obj) => {
        if (!obj) return '-/-/-';
        const low = obj.low !== null && obj.low !== undefined ? `$${obj.low}` : '-';
        const avg = obj.avg !== null && obj.avg !== undefined ? `$${obj.avg}` : '-';
        const high = obj.high !== null && obj.high !== undefined ? `$${obj.high}` : '-';
        return `${low}/${avg}/${high}`;
      };

      const formatAcos = (obj) => {
        if (!obj) return '-/-/-';
        const max = obj.max !== null && obj.max !== undefined ? obj.max : '-';
        const avg = obj.avg !== null && obj.avg !== undefined ? obj.avg : '-';
        const min = obj.min !== null && obj.min !== undefined ? obj.min : '-';
        return `${max}/${avg}/${min}`;
      };

      const ppcRow = `PPC: ${formatBid(item.ppc_bid)}`;
      const cpaRow = `CPA: ${formatBid(item.cpa)}`;
      const ppcCpaCol = `${ppcRow}<br>${cpaRow}`;

      const priceRow = `价: ${formatPrice(item.product_price)}`;
      const acosRow = `ACOS: ${formatAcos(item.acos)}`;
      const priceAcosCol = `${priceRow}<br>${acosRow}`;

      const budgetRow = `预算: ${item.ad_budget !== null && item.ad_budget !== undefined ? `$${item.ad_budget}` : '-'}`;
      const aba = [item.aba_concentration?.click_share, item.aba_concentration?.conversion_share].map(v).join('/');
      const budgetAbaCol = `${budgetRow}<br>ABA: ${aba}`;

      const topShares = (item.top_clicked_asins || [])
        .map(asin => [asin.click_share, asin.conversion_share].map(v).join('/'))
        .join('; ');

      lines.push(`| ${index + 1} | ${title} | ${searchesCol} | ${convCol} | ${ppcCpaCol} | ${priceAcosCol} | ${budgetAbaCol} | ${escapeMarkdownCell(topShares)} |`);
    });
  } else if (parsed.no_results) {
    lines.push('');
...
function normalizeProductTableData(tableData, maxCandidates) {
  ...
  const idx = {
    ...
    parentSales: findColumn(headers, [/销量/, /父|parent/i, /^(?!.*(?:趋势|trend))/i]),
    revenue: findColumn(headers, [/销售额|revenue/i, /^(?!.*(?:子体|child|趋势|trend))/i]),
    childSales: findColumn(headers, [/子体销量|子体销售额|child/i, /^(?!.*(?:趋势|trend))/i]),
  ...
  return applyOptionalLimit(rows, maxCandidates).map(row => {
    ...
    const buyboxText = valueAt(values, idx.buybox);
    const buyboxWords = buyboxText.split(/\s+/).filter(Boolean);
    const buybox_seller = buyboxWords[0] || null;
    const buybox_location = buyboxWords[1] || null;

    const lqsTokens = metricTokens(valueAt(values, idx.lqs));
    const weight_info = valueAt(values, idx.weight) || null;
    const pkg_weight_info = valueAt(values, idx.pkgWeight) || null;

    const result = {
      asin,
      category_bsr: bsrTokens[0] || null,
      bsr_growth_value: bsrTokens[1] || null,
      bsr_growth_rate: bsrTokens[2] || null,
      ...
      buybox_seller,
      buybox_location,
      lqs: lqsTokens[0] || null,
      weight_info,
      pkg_weight_info
    };

    if (row?.data) {
      if (row.data.title) result.title = row.data.title;
