...
3. **区间决策而非单点切割**：引擎输出的不是一个分数，而是一个**置信区间 [悲观, 乐观]**。只有当**乐观估计都不及格**时才淘汰；稳健地越过推荐线才直接推荐；其余一律作为 WATCHLIST 浮现给人工验证。这从数学上根除了"打回太多 + 触顶强出"。
...
2. **失真数据**：RECOMMEND+失真 → `LOOPBACK_TO_HAWKEYE`（重抓一次干净数据，预算耗尽则终态发布）；
